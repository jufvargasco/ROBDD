...
TEST_F(ManagerTest, Constructor_Test) {
    ...
    uTableVal *o = mg1->_getuTableVal(0);
    uTableVal *i = mg1->_getuTableVal(1);
    //ASSERT_EQ(o->label, "0");
    ASSERT_EQ(o->highV, 0);
    ASSERT_EQ(o->lowV, 0);
...
TEST_F(ManagerTest,createVar_Test){
    ...
    uTableVal *a = mg1->_getuTableVal(id);
    ASSERT_EQ(a->highV, 1);
    ASSERT_EQ(a->lowV, 0);
...
TEST_F(ManagerTest,isVariable_Test){
    ...
    BDD_ID id = mg1->createVar("a");
    uTableVal *a = mg1->_getuTableVal(id);
    std::cout <<"id: "<< id << " topvar: "<< a->topVar << " high: "<< a->highV << " low: "<< a->lowV << endl;
